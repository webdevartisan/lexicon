...
class CategoryModel extends AppModel
{
    ...
    public function findAllForAdmin(
        ...
        ?int $blogId = null,
        string $used = '',
        string $orderBy = 'b.blog_name ASC, c.name ASC'
    ): array {
        $page = max(1, $page);
        ...
        if ($q !== '') {
            $conditions[] = '(c.name LIKE :q_name OR c.slug LIKE :q_slug OR b.blog_name LIKE :q_blog)';
            $term = '%'.$q.'%';
            $params[':q_name'] = $term;
            $params[':q_slug'] = $term;
            $params[':q_blog'] = $term;
        ...
        if ($blogId !== null) {
            $conditions[] = 'c.blog_id = :blog_id';
            $params[':blog_id'] = $blogId;
        }

        // "Unused" is the one worth having: it is how you find taxonomy to prune.
        if ($used === 'yes') {
            $conditions[] = 'EXISTS (SELECT 1 FROM posts p WHERE p.category_id = c.id)';
        } elseif ($used === 'no') {
            $conditions[] = 'NOT EXISTS (SELECT 1 FROM posts p WHERE p.category_id = c.id)';
        }
