...
?>
<style>
  .post-engagement { display: flex; align-items: center; gap: .5rem; padding: 1.5rem 0; margin-top: 1.5rem; border-top: 1px solid var(--rule); position: relative; }
  .post-engagement .engage-btn { display: inline-flex; align-items: center; gap: .45rem; padding: .55rem 1.1rem; border: 1px solid var(--rule); border-radius: 999px; background: var(--plaster); color: var(--stone); font-family: var(--mono); font-size: var(--type-mono); letter-spacing: .12em; text-transform: uppercase; line-height: 1; cursor: pointer; transition: border-color .18s var(--ease-out), color .18s var(--ease-out), background .18s var(--ease-out); }
  .post-engagement .engage-btn:hover { border-color: var(--ink); color: var(--ink); }
  .post-engagement .engage-btn svg { width: 15px; height: 15px; display: block; }
  .post-engagement .engage-btn.is-active { border-color: var(--viridian); color: var(--viridian); background: var(--sage); }
  .post-engagement .engage-btn[data-engage="like"].is-active svg { fill: var(--viridian); }
  .post-engagement .engage-btn[data-engage="bookmark"].is-active svg { fill: var(--viridian); }
  .share-popover { position: absolute; bottom: calc(100% + 8px); left: 0; min-width: 215px; background: var(--mat); border: 1px solid var(--rule-strong); box-shadow: var(--frame-shadow); padding: .4rem; z-index: 50; }
  .share-popover a, .share-popover button { display: flex; align-items: center; gap: .55rem; width: 100%; padding: .55rem .65rem; border: 0; background: none; color: var(--ink-soft); font-family: var(--sans); font-size: var(--type-small); text-decoration: none; cursor: pointer; text-align: left; transition: background .12s ease; }
  .share-popover a:hover, .share-popover button:hover { background: var(--plaster-2); }
  ...
  .comment-replies .comment-item { font-size: 95%; margin-bottom: .75rem; border-top: 0; padding-block: .5rem; }
  .reply-toggle { font-family: var(--mono); font-size: var(--type-mono); letter-spacing: .12em; text-transform: uppercase; color: var(--stone); background: none; border: 0; padding: 0; cursor: pointer; text-decoration: none; }
  .reply-toggle:hover { color: var(--viridian); text-decoration: underline; }
  .reply-form { margin-top: .75rem; }
  .reply-form textarea { width: 100%; padding: .6rem; border: 1px solid var(--rule); background: transparent; font-family: var(--text); font-size: var(--type-small); }
  .reply-form textarea:focus { outline: 2px solid var(--viridian); outline-offset: 1px; }
  .reply-form .btn-reply { margin-top: .5rem; padding: .5rem 1.05rem; border: 1px solid var(--viridian); background: var(--viridian); color: var(--plaster); font-family: var(--mono); font-size: var(--type-mono); letter-spacing: .12em; text-transform: uppercase; cursor: pointer; }
  .reply-form .btn-reply:hover { background: var(--viridian-deep); border-color: var(--viridian-deep); }
