...
CREATE TABLE IF NOT EXISTS blogs (
    ...
    status_before_suspension VARCHAR(20) DEFAULT NULL COMMENT 'Status to restore when the owner suspension is lifted',
...
CREATE TABLE IF NOT EXISTS blog_settings (
    ...
    comments_auto_publish BOOLEAN NOT NULL DEFAULT TRUE COMMENT 'When on, new comments publish instantly; owner moderates retroactively',
...
CREATE TABLE IF NOT EXISTS comments (
    ...
    hidden_at TIMESTAMP NULL DEFAULT NULL COMMENT 'Soft-hidden from public reads without touching the moderation status',
    hidden_reason VARCHAR(32) DEFAULT NULL COMMENT 'Why it is hidden, e.g. author_suspended',
    ...
    INDEX idx_comment_hidden (hidden_at),
...
-- ----------------------------------------------------------------------------
-- The address on an account only moves once the NEW inbox proves control by
-- following a token, so a stolen session cannot silently redirect account
-- recovery to an attacker's address. One row per user: requesting another
-- change replaces the pending one.
-- ----------------------------------------------------------------------------
